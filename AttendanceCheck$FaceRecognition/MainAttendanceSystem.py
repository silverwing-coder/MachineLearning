...
def updateAttendance(sid):
    ...
    cursor = connector.cursor()
    cursor.execute(
        "SELECT total_attendance FROM Students WHERE id = '{}'".format(sid))
    number = cursor.fetchone()[0]
    dateTime = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
    # print("number: ", number)

    cursor.execute(
        "UPDATE Students SET total_attendance = {} WHERE id = '{}'".format(number+1, sid))
    cursor.execute(
        "UPDATE Students SET last_attendance = '{}' WHERE id = '{}'".format(dateTime, sid))
    cursor.execute("COMMIT")

    updatedNumber = cursor.execute(
        "SELECT total_attendance FROM Students WHERE id = '{}'".format(sid))
    # print("before: ", number)
    ...
    while True:
        ...
        if modeType == 3:
            ...
            (wt, ht), _ = cv2.getTextSize(
                str(studentInfo['name']), cv2.FONT_HERSHEY_SIMPLEX, 1, 1)
            ...
            if (frameCounter != 0):
                if frameCounter == 1:
                    ...
                    dateTimeObject = datetime.strptime(
                        studentInfo["last_attendance"], "%m/%d/%Y %H:%M:%S")
                    ...
                    if secondsElapsed > 30:
                        ...
                        modeType = 3
                        frameCounter = 0
                        imgBackground[40:40 + 640, 800:800 +
                                      420] = modeImgList[modeType]
                        ''' name display on center '''
                        (wt, ht), _ = cv2.getTextSize(
                            str(studentInfo['name']), cv2.FONT_HERSHEY_SIMPLEX, 1, 1)
                        offset = (420 - wt) // 2  # integer division
                        ...
                        cv2.putText(imgBackground, str(
                            studentInfo['name']), (800 + offset, 160), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
                ...
                if modeType == 1:
                    ...
                    if frameCounter <= 10:
                        ...
                        ''' name display on center '''
                        (wt, ht), _ = cv2.getTextSize(
                            str(studentInfo['name']), cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
                        offset = (420 - wt) // 2  # integer division
                        # print(offset)
                        cv2.putText(imgBackground, str(studentInfo['name']), (
                            820 + offset, 440), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
