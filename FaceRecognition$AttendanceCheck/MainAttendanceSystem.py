...
def getStudentInfo(sid):
    ...
    for row in cursor:
        ...
        studentInfo = {"id":row[0], "name": row[1], "major": row[2], "starting_year": row[3],
                       "total_attendance": row[4], "section": row[5], "year": row[6], "last_attendance": row[7]}
...
def webCamMain():
    ...
    file = open("EncodeFile.cis112", "rb")
    ...
    while True:
        ...
        if faceInCurrFrame:
            for encodeFace, faceLoc in zip(encodeCurrFrame, faceInCurrFrame):
                ...
                if matches[matchIndex]:
                    ...
                    ''' convert the imgSmall coordinates to the original image coordinates, then draw bounding box'''
                    y1, x2, y2, x1 = faceLoc
            ...
            if (frameCounter != 0):
                if frameCounter == 1:
                    ...
                    if secondsElapsed > 30:
                        ...
                        offset = (420 - wt) // 2  # integer division
                        # print(offset)
                        cv2.putText(imgBackground, str(studentInfo['name']), (800 + offset, 160), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 255, 255), 2)
                ...
                if modeType == 1:
                    ...
                    if frameCounter <= 10:
                        ...
                        #             0.5, (255, 255, 255), 2)
                        (wt1, ht1), _ = cv2.getTextSize(str(studentInfo['major']), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
                        offset1 = (420 - wt1) // 2  # integer division
                        cv2.putText(imgBackground, str(studentInfo['major']), (810+offset1, 500), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.5, (255, 255, 255), 2)
                        # cv2.putText(imgBackground, str(studentInfo['standing']), (900, 610), cv2.FONT_HERSHEY_SIMPLEX,
                        #             0.7, (50, 150, 50), 2)
                        cv2.putText(imgBackground, str(studentInfo['section']), (900, 610), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (50, 150, 50), 2)
                        ...
                        studentImgFile = 'Images/cis112/' + studentId + ".jpg"
                        studentImg = cv2.imread(studentImgFile)
                        studentImg = cv2.resize(studentImg, (220, 220))
        ...
        cv2.imshow("Attendance Check System", imgBackground)
